...
namespace Robotics::Polynomial {
    class CubicSpline2D {
      public:
         ...
         */
        void s(const std::vector<double>& x, const std::vector<double>& y);
                                 ...
                                 const std::size_t& interpolation_size)
    {
        ...
        std::transform(first_derivative_x.begin(), first_derivative_x.end(),
                       first_derivative_y.begin(), std::back_inserter(yaw),
                       [](double x, double y) { return std::atan2(y, x); });
        computeCurvature();
    }

    void CubicSpline2D::s(const std::vector<double>& x, const std::vector<double>& y)
    {
        ...
        std::transform(dy.cbegin(), dy.cend(), dx.cbegin(), std::back_inserter(m_s),
                       [](double yy, double xx) { return std::hypot(xx, yy); });

        std::partial_sum(m_s.begin(), m_s.end(), m_s.begin());
    }

    void CubicSpline2D::computeCurvature()
    {
        std::vector<double> ddydx;
        std::transform(second_derivative_y.begin(), second_derivative_y.end(),
                       first_derivative_x.begin(), std::back_inserter(ddydx),
                       std::multiplies<double>());

        std::vector<double> ddxdy;
        std::transform(second_derivative_x.begin(), second_derivative_x.end(),
                       first_derivative_y.begin(), std::back_inserter(ddxdy),
                       std::multiplies<double>());

        std::vector<double> diff_ddydx_ddxdy;
        std::transform(ddydx.begin(), ddydx.end(), ddxdy.begin(),
                       std::back_inserter(diff_ddydx_ddxdy), std::minus<double>());

        std::vector<double> dxdxdydy;
        std::transform(first_derivative_x.begin(), first_derivative_x.end(),
                       first_derivative_y.begin(), std::back_inserter(dxdxdydy),
                       [](double x, double y) { return x * x + y * y; });
        std::transform(diff_ddydx_ddxdy.begin(), diff_ddydx_ddxdy.end(), dxdxdydy.begin(),
                       std::back_inserter(curvature), std::divides<double>());
    }
}  // namespace Robotics::Polynomial
