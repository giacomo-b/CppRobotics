...
namespace Robotics::Polynomial{
  ...
  void CubicSpline::adjacentDiff(const std::vector<double>& in_vec,
                                 std::vector<double>& out_vec)
  {
      std::adjacent_difference(in_vec.cbegin(),
                               in_vec.cend(),
  ...
  void CubicSpline::computeConstantA()
  {
    std::copy(m_y.cbegin(),
              m_y.cend(),
              std::back_inserter(m_a));
  }

  void CubicSpline::computeDyDx()
  {
    m_dy_dx.reserve(n_points);
    std::transform(m_dy.begin(),
                   m_dy.end(),
                   m_dx.begin(),
                   std::back_inserter(m_dy_dx),
                   [](double a, double b)
                   {
                     return a / b;
                   });
  ...
  void CubicSpline::computeConstantB()
  {
    ...
    for(size_t idx = 0; idx < m_c.size() - 1; ++idx)
      {
        m_b.push_back(m_dy_dx.at(idx) -
                      m_dx.at(idx) *
                      (2 * m_c.at(idx) + m_c.at(idx + 1)) / 3
                      );
      }
  ...
  void CubicSpline::computeConstantC()
  {
    ...
    std::transform(m_dx.begin(),
                   m_dx.end() - 1,
                   m_dx.begin() + 1,
                   std::back_inserter(dx_sum),
                   std::plus<double>()
                   );
    ...
    std::adjacent_difference(m_dy_dx.begin(),
                             m_dy_dx.end(),
                             std::back_inserter(diff_dy_dx));
    ...
    C(0,0) = 0.0f;
    std::vector<double> cc(C.data(),
                          C.data() + C.rows() * C.cols());
    std::copy(cc.cbegin(),
              cc.cend(),
              std::back_inserter(m_c));
  }
  ...
  void CubicSpline::computeConstantD()
  {
    ...
    std::adjacent_difference(m_c.begin(),
                             m_c.end(),
                             std::back_inserter(diff_m_c)
                             );
    diff_m_c.erase(diff_m_c.begin());
    std::transform(diff_m_c.begin(),
                   diff_m_c.end(),
                   m_dx.begin(),
                   std::back_inserter(m_d),
                   [](double dc, double x)
                   {
                     return dc / (3*x);
                   });
  ...
  void CubicSpline::outputSplineSet()
  {
    for(size_t idx = 0; idx < n_points + 1; ++idx)
      {
        m_spline.emplace_back(SplineSet(m_a.at(idx),
                                        m_b.at(idx),
                                        m_c.at(idx),
                                        m_d.at(idx)
                                        )
  ...
  void CubicSpline::computeAllConstants()
  {
    ...
    std::transform(m_interval.begin(),
                   m_interval.end(),
    ...
    std::transform(m_interval.begin(),
                   ...
                   idx.begin(),
                   temp.begin(),
                   [&](double interv, size_t i)
                   {
                     return interv - m_x.at(i);
                   });
    std::transform(idx.begin(),
                   idx.end(),
                   std::back_inserter(m_val),
                   [&](size_t i)
                   {
  ...
  void CubicSpline::computePosition(const std::vector<double>& vec)
  {
    ...
    std::transform(m_val.begin(),
                   m_val.end(),
                   vec.begin(),
                   m_position.begin(),
                   [](SplineSet ss, double dx)
                   {
                     return ss.a + (ss.b * dx) + (ss.c * dx * dx) +
                       (ss.d * dx * dx * dx);
                   });
  }
  ...
  void CubicSpline::computeFirstDerivative(const std::vector<double>& vec)
  {
    ...
    std::transform(m_val.begin(), m_val.end(),
                   vec.begin(),
                   m_first_derivative.begin(),
                   [](SplineSet ss, double dx)
                   {
                     return ss.b + (2 * ss.c * dx) + (3 * ss.d * dx * dx);
                   });
  }
  void CubicSpline::computeSecondDerivative(const std::vector<double>& vec)
  {
    ...
    std::transform(m_val.begin(),
                   m_val.end(),
                   vec.begin(),
                   m_second_derivative.begin(),
                   [](SplineSet ss, double dx)
                   {
                     return (2 * ss.c) + (6 * ss.d * dx);
                   });
  }
  ...
  {
    auto upper = std::upper_bound(m_x.cbegin(),
                                  m_x.cend(),
                                  t);
